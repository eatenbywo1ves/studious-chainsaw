...
def run_enhanced_demonstration():
    ...
    print(f"Agent initialized with {agent.get_agent_status()['knowledge_base_size']} foundational concepts")
    ...
    for i, problem in enumerate(test_problems, 1):
        ...
        print(f"  Strategy Match: {'YES' if strategy_match else 'NO'} Expected: {problem['expected_strategy']}")
        print(f"  Execution Time: {execution_time:.3f}s")

        # Show Von Neumann insights
        insights = solution.get('von_neumann_insights', [])
        if insights:
            print(f"  Von Neumann Insights:")
    ...
    print(f"Overall Success Rate: {successful_problems}/{len(results)} ({successful_problems/len(results)*100:.1f}%)")
    print(f"Average Confidence: {avg_confidence:.3f}")
    print(f"Strategy Selection Accuracy: {strategy_matches}/{len(results)} ({strategy_matches/len(results)*100:.1f}%)")
    ...
    print("  + Self-Improvement - Continuous learning through stored program architecture")
