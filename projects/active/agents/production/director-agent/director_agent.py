...
class AgentRegistry:
    ...
    def get_best_agent(
        self, agent_type: AgentType, requirements: Dict[str, Any] = None
    ) -> Optional[str]:
    ...
    def update_agent_load(self, agent_id: str, load_change: int):
        if agent_id in self.agent_loads:
            self.agent_loads[agent_id] = max(
                0, self.agent_loads[agent_id] + load_change
            )
...
class DirectorAgent:
    ...
    async def create_project(
        self, user_prompt: str, video_specs: VideoSpecs, project_name: str = None
    ) -> str:
    ...
    async def _assign_and_execute_task(self, task: AgentTask):
        ...
        if not agent_id:
            self.logger.error(
                f"No available agent for task {task.id} of type {task.agent_type}"
            )
        ...
        for pid, project in self.active_projects.items():
            if (
                any(scene.id == task.scene_id for scene in project.scenes)
                or task.scene_id == "global"
            ):
                project_id = pid
            ...
            for task in project.tasks:
                if task.id == task_id:
                    ...
                    if task.assigned_agent_id:
                        self.agent_registry.update_agent_load(
    ...
    async def _update_project_progress(self, project_id: str):
        ...
        completed_tasks = sum(
            1 for task in project.tasks if task.status == TaskStatus.COMPLETED
        )
    ...
    async def _finalize_project(self, project_id: str):
        ...
        post_tasks = [
            t for t in project.tasks if t.agent_type == AgentType.POST_PRODUCTION
        ]
        if post_tasks:
            ...
            try:
                ...
                for agent_id, agent_info in list(self.agent_registry.agents.items()):
                    try:
                        ...
                        self.logger.warning(
                            f"Agent {agent_id} health check failed: {e}"
                        )
...
async def main():
    ...
    video_specs = VideoSpecs(
        duration=30.0, resolution=(1920, 1080), fps=30, style="cinematic"
    )
