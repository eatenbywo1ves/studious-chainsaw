        ...
        metrics = {
            ...
            "metrics": [
                ...
                },
                {
                    ...
                    "value": self.projects_managed,
                    "unit": "count",
                },
                {
                    "name": "agents_orchestrated",
                    "value": self.agents_orchestrated,
                    "unit": "count",
                },
                {
                    "name": "pipeline_executions",
                    "value": self.pipeline_executions,
                    "unit": "count",
                },
                {
                    "name": "resource_allocations",
                    "value": self.resource_allocations,
                    "unit": "count",
                },
                {
                    "name": "active_projects",
                    "value": len(self.active_projects),
                    "unit": "count",
                },
...
class ObservatoryEnabledDirectorAgent:
    ...
    def orchestrate_agent(
        self, agent_type: str, task: str, priority: int = 1, *args, **kwargs
    ):
    ...
    def execute_pipeline(self, pipeline_name: str, steps: List[str], *args, **kwargs):
        ...
        try:
            result = self.director.execute_pipeline(
                pipeline_name, steps, *args, **kwargs
            )
            return result
        except Exception as e:
    ...
    def allocate_resources(
        self, resource_type: str, amount: float, agent: str, *args, **kwargs
    ):
        self.adapter.on_resource_allocation(resource_type, amount, agent)

        try:
            result = self.director.allocate_resources(
                resource_type, amount, agent, *args, **kwargs
            )
            return result
        except Exception as e:
...
async def main():
    ...
    for i in range(3):
        ...
        try:
            # Example operations
            monitored_director.create_project(
                "video_generation", ["script_agent", "visual_agent"]
            )
            monitored_director.orchestrate_agent(
                "script_agent", f"generate_script_{i}", priority=1
            )
            monitored_director.execute_pipeline(
                "video_pipeline", ["script", "visual", "audio", "edit"]
            )
