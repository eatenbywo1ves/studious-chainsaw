    ...
    async def perform_agent_work(self, cycle):
        ...
        if self.agent_name == "VonNeumannAgent":
            ...
            await self.send_event(
                "reasoning_start", "info", f"Starting reasoning cycle {cycle}"
            )
            await asyncio.sleep(0.1)  # Simulate processing
            await self.send_event(
                "reasoning_complete", "info", f"Completed reasoning cycle {cycle}"
            )

            if cycle % 3 == 0:
                await self.send_event(
                    "synthesis", "info", f"Cross-domain synthesis event {cycle // 3}"
                )

        elif self.agent_name == "DirectorAgent":
            # Simulate orchestration tasks
            await self.send_event(
                "project_management", "info", f"Managing project cycle {cycle}"
            )
            await asyncio.sleep(0.1)  # Simulate processing
            ...
            if cycle % 4 == 0:
                await self.send_event(
                    "agent_orchestration",
                    "info",
                    f"Orchestrating agents for cycle {cycle}",
                )
...
async def main():
    ...
    tasks = [vonneumann_agent.connect_and_run(), director_agent.connect_and_run()]
