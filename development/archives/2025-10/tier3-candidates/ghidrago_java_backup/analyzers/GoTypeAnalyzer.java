...
public class GoTypeAnalyzer extends AbstractAnalyzer {
	...
	private boolean runPythonScript(Program program, TaskMonitor monitor, MessageLog log) {
		try {
			...
			// Create a script instance using GhidraScriptUtil
			GhidraScript script = GhidraScriptUtil.findScriptByName(scriptFile.getName());

			if (script != null) {
				// Set the script's program and monitor
				...
				log.appendMsg("Error: Could not load script: " + scriptFile.getName());
				return false;
			}
	...
	private File findPythonScript(String scriptName) {
		...
		ResourceFile[] scriptDirs = GhidraScriptUtil.getScriptSourceDirectories();
