...
def create_reference_glyphs():
    ...
    test_cases = [
        ...
        ("matrix_multiply_optimized", 8.0, 32.0, "gpu", {"smart_routed": True, "batched": True}, "Fully optimized compute"),

        # Shape: DIAMOND (Transform operations)
        ("xor_transform", 0.25, 0.1, "cpu", {"smart_routed": True, "speedup": 180}, "Fast XOR transform"),
        ("transform_base", 5.0, 0.5, "gpu", {}, "Base transform"),
        ("transform_optimized", 1.2, 0.3, "cpu", {"smart_routed": True, "batched": True}, "Optimized transform"),
        ...
        ("graph_algorithm_optimized", 25.0, 6.0, "hybrid", {"batched": True}, "Batched graph algorithm"),

        # Shape: STAR (Random generation)
        ("random_generation", 12.50, 2.0, "gpu", {}, "Base random generation"),
        ("random_generation_fast", 0.8, 1.0, "cpu", {"smart_routed": True}, "Fast random generation"),

        # Shape: TRIANGLE (Memory operations)
        ("memory_allocation", 35.0, 128.0, "gpu", {}, "GPU memory allocation"),
        ("memory_transfer", 18.0, 64.0, "hybrid", {"smart_routed": True}, "Optimized memory transfer"),
        ...
        ("ultra_fast", 0.1, 0.05, "cpu", {"smart_routed": True, "batched": True}, "Excellent performance"),
...
def generate_individual_svg_files(glyphs, output_dir="glyph_svgs"):
    ...
    for i, (glyph, description) in enumerate(glyphs, 1):
        ...
        print(f"[{i:2d}/{len(glyphs)}] {glyph.get_notation():<8} {glyph.operation_name:<35} -> {filename}")
...
def generate_reference_sheet_svg(glyphs, output_file="glyph_reference_sheet.svg"):
    ...
    for glyph, description in glyphs:
        ...
        marker_svg = renderer.render_iteration_marker_svg(
        ...
        # Operation name
        svg_parts.append(f'<text x="350" y="{y_offset + 25}" class="glyph-info">{glyph.operation_name}</text>')
        svg_parts.append(f'<text x="350" y="{y_offset + 42}" class="glyph-info" style="font-size: 10px; fill: #999;">{description}</text>')

        # Performance
        perf_label = f"{glyph.duration_ms:.2f}ms"
        if glyph.speedup:
            perf_label += f" ({glyph.speedup}x)"
        svg_parts.append(f'<text x="650" y="{y_offset + 25}" class="glyph-info">{perf_label}</text>')

        # Memory
        svg_parts.append(f'<text x="850" y="{y_offset + 25}" class="glyph-info">{glyph.memory_mb:.2f}MB ({glyph.size.value})</text>')

        # Device
        device_label = f"{glyph.device} ({glyph.border_style.value})"
        svg_parts.append(f'<text x="1000" y="{y_offset + 25}" class="glyph-info">{device_label}</text>')

        # Separator
        svg_parts.append(f'<line x1="50" y1="{y_offset + 60}" x2="1150" y2="{y_offset + 60}" stroke="#e0e0e0" stroke-width="1"/>')
    ...
    svg_parts.append(f'<text x="600" y="{height - 30}" class="subtitle" text-anchor="middle">Generated by Mernithian GPU Profiler - Phase 1 Visual Symbols</text>')
    svg_parts.append('')
...
def generate_shape_showcase_svg(output_file="glyph_shape_showcase.svg"):
    ...
    for i, (marker, symbol, desc) in enumerate(markers):
        x = 300 + i * 250
        svg_parts.append(f'<text x="{x}" y="130" class="marker-label" text-anchor="middle">{symbol}</text>')
        svg_parts.append(f'<text x="{x}" y="150" class="shape-label" text-anchor="middle" style="font-size: 11px;">{desc}</text>')

    # Render grid
    y_offset = 200
    for row, (shape, shape_name, shape_desc) in enumerate(shapes):
        # Row label
        svg_parts.append(f'<text x="100" y="{y_offset + 40}" class="shape-label">{shape_name}</text>')
        svg_parts.append(f'<text x="100" y="{y_offset + 58}" class="shape-label" style="font-size: 10px; fill: #999;">{shape_desc}</text>')
        ...
        for col, (marker, symbol, _) in enumerate(markers):
            ...
            shape_svg = renderer.render_shape_svg(
                ...
                40,
                color,
                DeviceStyle.GPU if row % 2 == 0 else DeviceStyle.CPU
            )
            svg_parts.append(shape_svg)
    ...
    svg_parts.append(f'<text x="700" y="{height - 30}" class="shape-label" text-anchor="middle" style="font-size: 12px; fill: #999;">Border: Solid=CPU, Dashed=GPU, Dotted=Hybrid | Colors: Performance-based encoding</text>')
    svg_parts.append('</svg>')
...
def print_glyph_table(glyphs):
    ...
    for glyph, description in glyphs:
        ...
        print(f"{glyph.get_notation():<10} {shape_name:<12} {opt_level:<12} "
