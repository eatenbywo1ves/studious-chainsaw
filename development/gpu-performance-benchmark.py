...
try:
    ...
    if cuda.is_available():
        ...
        for size in sizes:
            ...
            print(f"  Vector Add: {format_time(numba_time)} ({format_throughput(size, numba_time)})")
    ...
    for size in sizes:
        ...
        if size <= 2000:
            ...
            if n > 10:
                ...
                print(f"  Matrix Mul: {format_time(matmul_time)} ({format_throughput(ops, matmul_time)})")
...
try:
    ...
    if torch.cuda.is_available():
        ...
        for size in sizes:
            ...
            print(f"  Vector Add: {format_time(pytorch_time)} ({format_throughput(size, pytorch_time)})")
