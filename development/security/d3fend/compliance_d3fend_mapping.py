...
COMPLIANCE_D3FEND_MAPPINGS: List[ComplianceToD3FENDMapping] = [
    ...
    ComplianceToD3FENDMapping(
        ...
        d3fend_techniques=[D3FENDTechnique.ENCRYPTION_IN_TRANSIT],
    ...
    ComplianceToD3FENDMapping(
        ...
        d3fend_techniques=[D3FENDTechnique.ENCRYPTION_AT_REST],
...
class ComplianceD3FENDMapper:
    ...
    def __init__(self):
        self.mappings = {
            f"{m.control_framework}:{m.control_id}": m for m in COMPLIANCE_D3FEND_MAPPINGS
        }

    def get_d3fend_for_control(self, framework: str, control_id: str) -> ComplianceToD3FENDMapping:
        """Get D3FEND techniques for a compliance control"""
        key = f"{framework}:{control_id}"
        return self.mappings.get(key)

    def get_controls_for_technique(
        self, technique: D3FENDTechnique
    ) -> List[ComplianceToD3FENDMapping]:
        """Get all compliance controls that map to a D3FEND technique"""
        return [m for m in COMPLIANCE_D3FEND_MAPPINGS if technique in m.d3fend_techniques]

    def get_framework_coverage(self, framework: str) -> Dict[str, any]:
        """Get D3FEND coverage for a compliance framework"""
        framework_mappings = [
            m for m in COMPLIANCE_D3FEND_MAPPINGS if m.control_framework == framework
        ]
    ...
    def generate_implementation_checklist(
        self, framework: str, implemented_techniques: List[str]
    ) -> Dict[str, any]:
