...
class RefreshRequest(BaseModel):
    ...
    refresh_token: str = Field(..., min_length=10, max_length=2000, description="JWT refresh token")
...
class LatticeCreateRequest(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Lattice name (optional)"
    )
    dimensions: int = Field(..., ge=1, le=10, description="Number of dimensions (1-10)")
    size: int = Field(..., ge=2, le=100, description="Lattice size per dimension (2-100)")
...
class PathFindRequest(BaseModel):
    ...
    lattice_id: str = Field(..., min_length=1, max_length=100, description="Lattice identifier")
    start: List[int] = Field(
        ..., min_length=1, max_length=10, description="Start coordinates (1-10 dimensions)"
    )
    end: List[int] = Field(
        ..., min_length=1, max_length=10, description="End coordinates (1-10 dimensions)"
    )
...
class TenantCreateRequest(BaseModel):
    ...
    company_name: str = Field(
        ..., min_length=1, max_length=200, description="Company/organization name"
    )
    slug: str = Field(
        ..., min_length=3, max_length=50, pattern=r"^[a-z0-9-]+$", description="Unique tenant slug"
    )
    contact_email: EmailStr = Field(..., description="Primary contact email")
    subscription_plan: Optional[str] = Field(
        None, min_length=1, max_length=50, description="Subscription plan name"
    )
...
class UserCreateRequest(BaseModel):
    ...
    role: Optional[str] = Field("user", pattern=r"^(admin|user|viewer)$", description="User role")
...
class PaginationParams(BaseModel):
    ...
    limit: int = Field(20, ge=1, le=100, description="Number of items per page (1-100)")
...
class ExportRequest(BaseModel):
    ...
    format: str = Field("json", pattern=r"^(json|csv|excel)$", description="Export format")
    fields: Optional[List[str]] = Field(
        None, min_length=1, max_length=50, description="Fields to export"
    )
