...
def main():
    ...
    success, output = run_command(
        f'python "{script_dir / "run-security-audit.py"}" {env}', "Security audit (200+ checks)"
    )
    ...
    success, output = run_command(
        f'python "{script_dir / "run-pentest-offline.py"}"', "Penetration testing suite"
    )
    ...
    report_file = (
        project_root / f"deployment-report-{env}-{datetime.now().strftime('%Y%m%d-%H%M%S')}.txt"
    )
