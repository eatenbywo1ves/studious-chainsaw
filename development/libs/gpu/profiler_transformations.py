    ...
    def is_applicable(self, context: Dict[str, Any]) -> bool:
        ...
        for cond in self.prohibited_conditions:
            if self._check_condition(cond, context):
                logger.debug(f"Transformation {self.name}: prohibited condition {cond.value} present")
                return False
...
class TransformationCatalog:
    ...
    def _initialize_standard_rules(self):
        ...
        self.add_rule(TransformationRule(
            ...
            required_conditions=[
                TransformationCondition.TOLERANCE_RELAXED
            ],
            prohibited_conditions=[
                TransformationCondition.PRECISION_CRITICAL
            ],
    ...
    def verify_transformation(
        ...
        if after_time > 0:
            ...
            if speedup < min_expected * 0.5:
                ...
                report['warnings'].append(
                    f"Speedup {speedup:.1f}x exceeds expected range "
        ...
        for rule_name, rule in self.rules.items():
            if rule.verification_result:
                ...
                if rule.performance_guarantee:
                    ...
                    lines.append(f"    Performance Guarantee: {guarantee.bound}x {guarantee.bound_type} {guarantee.guarantee_type}")
