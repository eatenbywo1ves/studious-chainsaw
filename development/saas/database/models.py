...
class Tenant(Base):
    ...
    usage_metrics = relationship(
        "UsageMetric", back_populates="tenant", cascade="all, delete-orphan"
    )
...
class BillingRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('pending', 'paid', 'failed', 'refunded')", name="valid_billing_status"
        ),
...
class TenantLattice(Base):
    ...
    operations = relationship(
        "LatticeOperation", back_populates="lattice", cascade="all, delete-orphan"
    )
...
class LatticeOperation(Base):
    ...
    __table_args__ = (Index("idx_lattice_operations_tenant", "tenant_id", "created_at"),)
