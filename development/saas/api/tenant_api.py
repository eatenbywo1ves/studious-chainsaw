...
async def register_tenant(
    ...
    try:
        ...
        plan = (
            ...
            .filter_by(code=registration.plan_code, is_active=True)
            .first()
        )
        ...
        background_tasks.add_task(
            send_welcome_email, registration.email, registration.first_name, tenant.slug
        )
        ...
        return {
            ...
            "user": {"id": str(owner.id), "email": owner.email, "role": owner.role},
            "tokens": tokens.dict(),
        }
...
async def get_current_tenant(
    ...
    tenant = (
        db.query(Tenant)
        .filter_by(id=UUID(current_user.tenant_id), status=TenantStatus.ACTIVE)
        .first()
    )
...
async def update_tenant(
    updates: dict, current_user: TokenData = Depends(require_admin), db: Session = Depends(get_db)
):
    """Update tenant information"""

    tenant = db.query(Tenant).filter_by(id=UUID(current_user.tenant_id)).first()
...
async def create_user(
    ...
    existing = (
        db.query(User)
        .filter_by(tenant_id=UUID(current_user.tenant_id), email=user_data.email)
        .first()
    )
...
async def list_users(
    ...
    users = (
        db.query(User)
        .filter_by(tenant_id=UUID(current_user.tenant_id))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
async def update_user(
    ...
    if updates.role and user.role == UserRole.OWNER:
        owner_count = (
            db.query(User)
            .filter_by(tenant_id=UUID(current_user.tenant_id), role=UserRole.OWNER)
            .count()
        )
...
async def delete_user(
    user_id: UUID, current_user: TokenData = Depends(require_admin), db: Session = Depends(get_db)
):
...
async def create_api_key(
    ...
    api_key, key_hash = generate_api_key(
        tenant_id=current_user.tenant_id, name=key_data.name, permissions=key_data.permissions
    )
...
async def list_api_keys(
    ...
    keys = db.query(ApiKey).filter_by(tenant_id=UUID(current_user.tenant_id), is_active=True).all()
...
async def get_usage_stats(
    ...
    api_calls = (
        ...
        .filter_by(tenant_id=UUID(current_user.tenant_id), metric_type="api_calls")
        .filter(UsageMetric.period_start >= period_start, UsageMetric.period_end <= period_end)
        .all()
    )
    ...
    lattices_created = (
        db.query(TenantLattice)
        .filter_by(tenant_id=UUID(current_user.tenant_id))
        .filter(TenantLattice.created_at >= period_start, TenantLattice.created_at < period_end)
        .count()
    )
...
async def update_subscription(
    ...
    subscription = (
        ...
        .filter_by(tenant_id=UUID(current_user.tenant_id), status=SubscriptionStatus.ACTIVE)
        .first()
    )
    ...
    # Get new plan
    new_plan = db.query(SubscriptionPlan).filter_by(code=update.plan_code, is_active=True).first()

    if not new_plan:
        raise HTTPException(status_code=400, detail=f"Invalid plan code: {update.plan_code}")
    ...
    try:
        db.commit()
        return {"message": "Subscription updated successfully", "new_plan": new_plan.name}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Update failed: {str(e)}")
