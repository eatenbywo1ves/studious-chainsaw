...
export async function GET(request: NextRequest) {
  try {
    ...
    if (customerId) {
      ...
      // Note: Stripe doesn't support filtering by metadata in list, so we search all customers
      const allCustomers = await stripe.customers.list({
        limit: 100
      });

      const customers = {
        data: allCustomers.data.filter(c => c.metadata?.user_id === userId)
      };
    ...
    const formattedSubscriptions = subscriptions.data.map(sub => {
      ...
      customer: typeof sub.customer === 'object' && !('deleted' in sub.customer) ? {
        id: sub.customer.id,
