    ...
    while time.time() - start_time < duration_seconds:
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"\r[MEASURE] Progress: {elapsed}/{duration_seconds}s | "
...
def generate_report(stats: dict, metrics: List[GPUMetrics]):
    ...
    print(f"  - Range: {stats['gpu_utilization']['min']:.1f}% - {stats['gpu_utilization']['max']:.1f}%")
    ...
    print(f"  - Range: {stats['memory_utilization']['min']:.1f}% - {stats['memory_utilization']['max']:.1f}%")
    print(f"  - P95:   {stats['memory_utilization']['p95']:.1f}%")

    print("\nTemperature:")
    print(f"  - Mean: {stats['temperature']['mean']:.1f}°C")
    print(f"  - Max:  {stats['temperature']['max']}°C")
    print(f"  - Status: {stats['temperature']['status']} (threshold: {stats['temperature']['safe_threshold']}°C)")
