...
from libs.gpu import get_transformation_catalog
...
def test_small_gpu_to_cpu_transformation():
    ...
    print(
        f"  Context: {applicable_context['data_size_mb']:.1f}MB, "
        f"{applicable_context['transfer_time_ratio'] * 100:.0f}% transfer time"
    )
    print(f"  Estimated speedup: {speedup:.1f}x")
    print(
        f"  Expected range: {rule.expected_speedup_range[0]:.1f}x - "
...
def test_transformation_applicability():
    ...
    assert any(r.name == "SmallGPUToCPU" for r, _ in applicable1), "Should suggest SmallGPUToCPU"

    # Test scenario 2: Multiple batching opportunity
    context2 = {"similar_ops_count": 20, "has_dependencies": False, "data_size_mb": 10.0}
    ...
    assert any(r.name == "BatchFusion" for r, _ in applicable2), "Should suggest BatchFusion"
    ...
    assert any(r.name == "LargeCPUToGPU" for r, _ in applicable3), "Should suggest LargeCPUToGPU"
...
def test_transformation_verification():
    ...
    before_metrics = {"duration_ms": 10.0, "memory_peak_mb": 0.5, "error": None}
...
def test_optimizer_integration():
    ...
    assert len(transformation_suggestions) > 0, "Should have suggestions with transformation rules"
