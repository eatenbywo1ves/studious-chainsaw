...
def benchmark_matrix_operation(framework, operation, sizes, warmup_runs=3, test_runs=5):
    ...
    for size in sizes:
        ...
        try:
            ...
            print(
                f"  {size:4d}x{size:<4d}: {avg_time * 1000:6.2f}ms | {gflops:6.1f} GFLOPS | {bandwidth_gbps:5.1f} GB/s"
            )
...
for name, results in frameworks:
    if results:
        ...
        if optimal_size:
            ...
            print(
                f"  Efficiency: {optimal_gflops / GTX_1080_SPECS['theoretical_fp32_gflops'] * 100:.1f}% of theoretical peak"
            )
...
print(
    f"1. Best overall: {best_overall['size']}x{best_overall['size']} with {best_overall['framework']}"
)
