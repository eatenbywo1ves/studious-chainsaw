...
class TestReactiveWebhookPipeline:
    ...
    def test_priority_grouping_with_marble_diagram(self):
        ...
        source = scheduler.create_hot_observable(
            ReactiveTest.on_next(10, high1),     # -H
            ReactiveTest.on_next(20, normal1),   # -N
            ReactiveTest.on_next(30, low1),      # -L
            ReactiveTest.on_next(40, urgent1),   # -U
            ReactiveTest.on_next(50, high2),     # -H
            ReactiveTest.on_next(60, normal2),   # -N
        ...
        # Run the test
        results = scheduler.start(create_pipeline)

        # Verify we got only HIGH and URGENT events in order
        assert len(results.messages) == 4  # high1, urgent1, high2, completed
        assert results.messages[0].value.value.event_id == "h1"
        assert results.messages[1].value.value.event_id == "u1"
        assert results.messages[2].value.value.event_id == "h2"
        assert results.messages[3].value.kind == 'C'  # Completion
    ...
    def test_circuit_breaker_filtering_with_marble_diagram(self):
        ...
        circuit_states = {
            ...
            20: CircuitBreakerState.OPEN,
            30: CircuitBreakerState.CLOSED,
            40: CircuitBreakerState.CLOSED,
            50: CircuitBreakerState.CLOSED,
            60: CircuitBreakerState.CLOSED,
        }
        ...
        def mock_circuit_breaker_check(url: str) -> bool:
            ...
            return circuit_states.get(current_time_box['time'], CircuitBreakerState.CLOSED) == CircuitBreakerState.CLOSED
        ...
        print("✓ Circuit breaker filtering test passed!")
    ...
    def test_retry_logic_with_marble_diagram(self):
        ...
        # Test with retry(3) - should attempt 4 times total (1 + 3 retries)
        results = []
        ...
        assert len(results) == 4  # Original + 3 retries
    ...
    def test_backpressure_with_buffer(self):
        ...
        source = scheduler.create_hot_observable(
            ...
            ReactiveTest.on_next(70, "d"),
            ReactiveTest.on_next(80, "e"),
            ReactiveTest.on_next(90, "f"),
        ...
        print("✓ Backpressure buffering test passed!")
