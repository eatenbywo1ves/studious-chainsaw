...
from libs.gpu import get_glyph_analyzer, GlyphDescriptor, GlyphRenderer
...
class SVGGalleryGenerator:
    ...
    def generate_timeline_svg(
        ...
        for glyph, description in glyphs:
            ...
            svg_parts.append(
                f'<text x="220" y="{y_offset + 20}" class="timeline-label">{glyph.get_notation()}</text>'
            )
            svg_parts.append(
                f'<text x="280" y="{y_offset + 20}" class="timeline-label">{glyph.operation_name}</text>'
            )
            ...
            bar_x = 800
            svg_parts.append(
                f'<rect x="{bar_x}" y="{y_offset + 5}" width="{bar_width}" height="30" '
                f'fill="{glyph.color}" opacity="0.6" stroke="#333" stroke-width="1"/>'
            )

            # Time label
            svg_parts.append(
                f'<text x="{bar_x + bar_width + 10}" y="{y_offset + 25}" '
                f'class="timeline-time">{glyph.duration_ms:.2f}ms</text>'
            )

            # Memory label
            svg_parts.append(
                f'<text x="1450" y="{y_offset + 25}" '
        ...
        svg_parts.append(
            f'<text x="{width / 2}" y="{height - 30}" class="timeline-label" '
    ...
    def generate_performance_heatmap_svg(
        self, glyphs: List[Tuple[GlyphDescriptor, str]], output_file: str
    ):
        ...
        for level_idx in range(4):
            ...
            # Level box
            svg_parts.append(
                f'<rect x="{x_offset}" y="{y_offset}" width="640" height="320" '
                f'fill="{level_colors[level_idx]}" stroke="#999" stroke-width="2" rx="10"/>'
            )

            # Level title
            svg_parts.append(
                f'<text x="{x_offset + 320}" y="{y_offset + 30}" '
            ...
            for i, (glyph, desc) in enumerate(glyphs_in_level[:15]):  # Max 15 per level
                ...
                svg_parts.append(
                    f'<text x="{glyph_x + 25}" y="{glyph_y + 75}" '
    ...
    def export_all_formats(self, json_file: str, output_prefix: str = "export"):
        ...
        self.generate_timeline_svg(
            glyphs, timeline_file, title=data.get("session_name", "GPU Profiling Timeline")
        )
...
Examples:
  ...
  python glyph_svg_exporter.py proof_library.json --output proofs
    ...
    parser.add_argument(
        "--output", "-o", default="export", help="Output file prefix (default: export)"
    )
    parser.add_argument(
        "--timeline-only", action="store_true", help="Generate only timeline visualization"
    )
    parser.add_argument(
        "--heatmap-only", action="store_true", help="Generate only heatmap visualization"
    )
