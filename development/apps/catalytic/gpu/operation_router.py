...
class OperationAnalyzer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.gpu_threshold = self.config.get(
            'gpu_threshold',
            self.settings.gpu.gpu_threshold_elements if hasattr(self.settings, 'gpu') else self.DEFAULT_GPU_THRESHOLD
        )
        self.gpu_optimal_threshold = self.config.get(
            'gpu_optimal_threshold',
            self.settings.gpu.gpu_optimal_threshold if hasattr(self.settings, 'gpu') else self.DEFAULT_GPU_OPTIMAL_THRESHOLD
        )

        # Runtime statistics for adaptive routing
        self._operation_stats: Dict[OperationType, Dict] = {}

        logger.info(f"OperationAnalyzer initialized: gpu_threshold={self.gpu_threshold}, "
    ...
    def should_use_gpu(
        ...
        if available_gpu_memory_mb is not None:
            if characteristics.memory_estimate_mb > available_gpu_memory_mb * 0.8:
                return False, DevicePreference.CPU_OPTIMAL, \
        ...
        if characteristics.operation_type in [OperationType.GRAPH_ALGORITHM,
                                               OperationType.PATH_FINDING]:
            return False, DevicePreference.CPU_OPTIMAL, \
        ...
        if op_speedup > 5:  # Good speedup
            return True, DevicePreference.GPU_ACCEPTABLE, \
    ...
    def route_operation(
        ...
        **kwargs
        ...
        logger.debug(f"Routing decision: {operation_type.value} -> "
