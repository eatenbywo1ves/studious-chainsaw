...
class CatalyticLatticeGPU:
    ...
    def __init__(self, dimensions: int, size: int, device: str = "cuda"):
        ...
        print(
            f"[PIRATE FLAG] Lattice initialized! Memory allocated: {self.get_memory_usage():.2f} MB"
        )
    ...
    def compute_laplacian(self, field: torch.Tensor) -> torch.Tensor:
        ...
        if self.dimensions == 2:
            ...
            kernel = (
                torch.tensor(
                    [[0, 1, 0], [1, -4, 1], [0, 1, 0]], device=self.device, dtype=field.dtype
                )
                .unsqueeze(0)
                .unsqueeze(0)
            )
...
def benchmark_comparison():
    ...
    if results:
        ...
        print(
            f"  [PIRATE] GTX 1080 Total Memory: {torch.cuda.get_device_properties(0).total_memory / (1024**3):.1f} GB"
        )

        print("\n  Detailed Results:")
        for r in results:
            print(
                f"    {r['config']:10s} - Speedup: {r['speedup']:5.2f}x, "
